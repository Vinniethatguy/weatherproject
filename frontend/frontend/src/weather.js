...
function Weather({ location, dayData }) {
  ...
  return (
    <Container>
      <Row>
        <Col className='weather_card' xs={2} orderFirst>
          <div className='weather_info'>
            ...
            <span>{location}</span>
            <br />
            <img className="weather-image" src={currentDay[5]} alt='' />
            <span className='temp'>{currentDay[3]}</span>
          </div>
        </Col>
        <Col className='weekly_forcast' orderLast>
            ...
            {nextFourDays.map((day, index) => (
              <div key={index} className="image-wrapper">
                <Card style={{ width: '8rem', backgroundColor: '#005F73', color: '#FFFFFF', width: '5.5rem'}}>
                  ...
                  <span>{location}</span>
                  <br />
                  <img className="weekly-images" src={day[5]} alt=''  />
                  <br />
